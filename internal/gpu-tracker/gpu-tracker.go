...
// initialize GPU Tracker
type initializeGPUTrackerType func(string) error
...
func acquireLock(lockFile string) (*flock.Flock, error) {
	...
	for {
		...
		case <-timeout:
			...
			fmt.Printf("Acquiring lock timeout exceeded\n")
			return nil, fmt.Errorf("lock acquisition timed out")
		case <-tick:
...
func isGPUTrackerInitialized() (bool, error) {
	...
	if err == nil {
		...
		if !os.IsNotExist(err) {
			fmt.Printf("Error checking file %v, Error:%v\n", gpuTrackerFile, err)
			return false, err
		}
	}
...
func readGPUTrackerFile() (gpu_tracker_data_t, error) {
	...
	if err != nil {
		...
		fmt.Printf("Error opening file, Error: %v\n", err)
		return gpu_tracker_data_t{GPUsStatus: make(map[int]gpu_status_t), GPUsInfo: make(map[int]amdgpu.DeviceInfo)}, err
	}
	defer file.Close()
	...
	if err := decoder.Decode(&gpuTrackerData); err != nil {
		...
		fmt.Printf("Failed to decode JSON, Error: %v\n", err)
		return gpu_tracker_data_t{GPUsStatus: make(map[int]gpu_status_t), GPUsInfo: make(map[int]amdgpu.DeviceInfo)}, err
	}
...
func writeGPUTrackerFile(gpuTrackerData gpu_tracker_data_t) error {
	...
	if err != nil {
		...
		fmt.Printf("Error creating temp file, Error: %v\n", err)
		return err
	}
	...
	if err := encoder.Encode(gpuTrackerData); err != nil {
		...
		fmt.Printf("Error encoding JSON to temp file, Error: %v\n", err)
		return err
	}
	...
	if err := tempFile.Sync(); err != nil {
		...
		fmt.Printf("Error syncing temp file: %v\n", err)
		return err
	}
...
func initializeGPUTracker(op string) error {
	gpusInfo, err := amdgpu.GetAMDGPUs()
	if err != nil {
		logger.Log.Printf("Failed to get AMD GPUs info, Error: %v", err)
		fmt.Printf("Failed to get AMD GPUs info, Error: %v\n", err)
	...
	gpuTrackerData := gpu_tracker_data_t{GPUsStatus: make(map[int]gpu_status_t), GPUsInfo: make(map[int]amdgpu.DeviceInfo)}
	...
	err = writeGPUTrackerFile(gpuTrackerData)
	if err != nil {
		logger.Log.Printf("Failed to %v GPU Tracker, Error: %v", op, err)
		fmt.Printf("Failed to %v GPU Tracker, Error: %v\n", op, err)
		return err
	}
...
func (gpuTracker *gpu_tracker_t) Init() error {
	...
	err = gpuTracker.initializeGPUTracker("Initialize")
	if err == nil {
		logger.Log.Printf("GPU Tracker has been initialized")
		fmt.Printf("GPU Tracker has been initialized\n")
	}
...
func (gpuTracker *gpu_tracker_t) Reset() error {
	...
	err = gpuTracker.initializeGPUTracker("Reset")
	if err == nil {
		logger.Log.Printf("GPU Tracker has been reset")
		fmt.Printf("GPU Tracker has been reset\n")
		fmt.Printf("It is recommended to stop and restart running containers to get the most accurate GPU Tracker status\n")
	}
...
func (gpuTracker *gpu_tracker_t) ShowStatus() error {
	...
	if gpuTrackerInitialized {
		...
		fmt.Printf("GPU Tracker is not initialized\n")
	}
