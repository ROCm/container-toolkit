...
func (rt *runtm) Run() error {
	...
	if rt.oci.IsCreate() {
		// Check if hook mode is enabled via environment or config
		useHookMode := os.Getenv("AMD_RUNTIME_HOOK_MODE") == "1"
		
		if useHookMode {
			// Hook mode: register prestart hook, let hook handle device injection
			err = rt.oci.UpdateSpec(oci.AddPrestartHook)
			if err != nil {
				logger.Log.Printf("Failed to add prestart hook, Error: %v", err)
				return err
			}
			logger.Log.Printf("Using hook mode for GPU device injection")
		} else {
			// Legacy mode: direct device injection (current behavior)
			err = rt.oci.UpdateSpec(oci.AddGPUDevices)
			if err != nil {
				logger.Log.Printf("Failed to add Linux device into OCI spec, Error: %v", err)
				return err
			}
			logger.Log.Printf("Using direct mode for GPU device injection")
		}
